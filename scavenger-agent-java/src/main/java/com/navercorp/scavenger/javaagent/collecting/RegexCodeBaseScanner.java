...
@Log
public class RegexCodeBaseScanner implements CodeBaseScanner {
    private final Config config;

    private final Pattern packageRegex;
    private final Pattern excludePackageRegex;
    private final Pattern additionalPackageRegex;

    public RegexCodeBaseScanner(Config config) {
        this.config = config;
        this.packageRegex = Pattern.compile(config.getPackageRegex());
        if (config.getAdditionalPackageRegex().isEmpty()) {
            this.additionalPackageRegex = null;
        } else {
            this.additionalPackageRegex = Pattern.compile(config.getAdditionalPackageRegex());
        }
        if (config.getExcludePackageRegex().isEmpty()) {
            this.excludePackageRegex = null;
        } else {
            this.excludePackageRegex = Pattern.compile(config.getExcludePackageRegex());
        }
    }
    ...
    private List<ClassNode> scanClasses(JarInputStream jarInputStream) throws IOException {
        ...
        while ((innerEntry = jarInputStream.getNextJarEntry()) != null) {
            if (innerEntry.getName().startsWith("/BOOT-INF/classes/com/example/demo")) {
    ...
    private boolean containsPackageNameInPath(String path, String separator) {
        ...
        // 원래 포함된 친구들... : BOOT-INF/classes/com/example/demo/**
        // jar에서 포함된 친구들... : com/example/demo/**
        // matches로 할 경우 이슈... ^ 처리는 어떻게 할 것인지...
        return packageRegex.matcher(location).find();
    }
    ...
    private boolean isIncludedByPackage(ClassNode clazz) {
        final String replaceSlashToDot = clazz.name.replace("/", ".");
        boolean includedByPackage = packageRegex.matcher(replaceSlashToDot).matches();
        if (includedByPackage) {
            boolean excludedByPackage = excludePackageRegex != null && excludePackageRegex.matcher(replaceSlashToDot).matches();
    ...
    private boolean isAdditionalPackage(ClassNode clazz) {
        final String replaceSlashToDot = clazz.name.replace("/", ".");
        return additionalPackageRegex != null && additionalPackageRegex.matcher(replaceSlashToDot).matches();
    }
